...
unsigned char global_key []= { 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37,
...
unsigned char global_iv []= { 0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37,
...
void write_students_to_file_encrypted();
...
void get_excellent_students() {
    ...
    fprintf(stdout,"Enter the number of course that you want to know its excellent students: ");

    fscanf(stdin, "%d", &course_num);
    ...
    for (int j = 0; j < NUM_LEVELS; j++) {
        ...
        current = max_grades[course_num][j];

        int counter = 0;

        while (current != NULL && counter < NUM_OF_EXCELLENCE) {



            printf("\tStudent: %s %s, Grade: %d, ranked: %d\n",

                   current->stud->first_name, current->stud->last_name,

                   current->stud->courses[course_num], counter);
...
void write_students_to_file_encrypted() {
    ...
    FILE* file = fopen("students_encrypted.txt", "wb");
    ...
    for (int level = 0; level < NUM_LEVELS; level++) {
        ...
        for (int class = 0; class < NUM_CLASSES; class++) {
            ...
            while (current != NULL) {
                ...
                unsigned char data_line[128];

                int data_len = sprintf((char*)data_line, "%s %s %s %d %d %d %d %d %d %d %d %d %d %d %d\n", current->first_name,

                                       current->last_name, current->cellphone, current->level, current->class,

                                       current->courses[0], current->courses[1], current->courses[2], current->courses[3],

                                       current->courses[4], current->courses[5], current->courses[6], current->courses[7],

                                       current->courses[8], current->courses[9]);
                ...
                ciphertext_len = encrypt(data_line, strlen ((char *)data_line), global_key, global_iv, ciphertext);



                //fprintf(file, "%d%s", ciphertext_len,ciphertext);
...
void read_encrypted_file() {
    ...
    FILE* file = fopen("students_data_out_encrypted.txt", "rb");
    ...
    while (fread(&ciphertext_len, sizeof(int), 1, file) == 1) {
        ...
        fread(ciphertext, sizeof(unsigned char), ciphertext_len, file);



        /* Decrypt the data */

        unsigned char decryptedtext[128];  // Make sure this is large enough

        int plaintext_len = decrypt(ciphertext, ciphertext_len, global_key, global_iv, decryptedtext);
        ...
        /* Now decryptedtext contains the decrypted data */
